...
class Ui(metaclass=SingletonMeta):
    ...
    def draw_health_bar_player(self, jugador, pantalla):
        ...
        primer_sprite = self.barras[0] if segmentos_danados == num_segmentos else self.barras[3]
        ultimo_sprite = self.barras[2] if segmentos_danados > 0 else self.barras[5]

        surface_bar.blit(primer_sprite, (0, 0))  # Primer segmento

        for i in range(num_segmentos - 2):
            sprite = self.barras[1] if i + 1 >= num_segmentos - segmentos_danados else self.barras[4]
            surface_bar.blit(sprite, ((i + 1) * sprite_width, 0))
